...
Several modifications has been made in \texttt{neighbours}, \texttt{find6Neighbors} and \texttt{find18Neighbours} function to remove an unnecessary intermediate \texttt{vector}. Instead, the pointer of the output memory space was passed directly to \texttt{findNNeighbors} functions. \lref{lst:neighbors} shows the modification of the code. An additional $10.87\%$ speedup was obtained.
...
\begin{lstlisting}[language=diff,label=lst:find,caption={\texttt{findNeighbours} code modification},captionpos=b, float, floatplacement=t]
